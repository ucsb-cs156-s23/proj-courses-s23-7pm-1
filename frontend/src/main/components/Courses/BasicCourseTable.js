...
const BasicCourseTable = ({ classes, checks, displayQuarter, allowExport }) => {
  ...
  const renderCourseId = (_cell, row) => {
    const courseId = row.course.courseId;
    return (courseId)
  }
  ...
  const renderDescription = (_cell, row) => {
    const description = row.course.description;
    return (description);
  }

  const renderSubjectArea = (_cell, row) => {
    const subjectArea = row.course.subjectArea;
    return (subjectArea);
  }

  const renderLevelCode = (_cell, row) => {
    const levelCode = row.course.objLevelCode;
    return (levelCode);
  }


  //*************this will need to be changed likely */
  ...
  const columns = [{
    ...
    text: 'Description',
    formatter: (cell, row) => renderDescription(cell, row),
    csvFormatter: (cell, row) => renderDescription(cell, row)
  },{
    dataField: 'course.subjectArea',
    text: 'Subject Area',
    formatter: (cell, row) => renderSubjectArea(cell, row),
    csvFormatter: (cell, row) => renderSubjectArea(cell, row)
  }, {
    dataField: 'course.objLevelCode',
    text: 'Level Code',
    formatter: (cell, row) => renderLevelCode(cell, row),
    csvFormatter: (cell, row) => renderLevelCode(cell, row)
  },{
